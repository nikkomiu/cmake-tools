...
When Doxygen is used and the binary exists on the machine, there are a number of additional/supporting
documentation files that can be included as part of the documentation build. Doxygen outlines how to
use these within their [Additional Documentation](https://doxygen.nl/manual/additional.html) docs.

For additional documentation pages, you can include Markdown and plain text files in a `docs/`
subdirectory of the project. For Markdown files the `.md` and `.markdown` file extensions are
supported. Doxygen supports multiple syntax guidelines for Markdown outlined in their
[Markdown](https://doxygen.nl/manual/markdown.html) docs. For plain text files, `.txt` files
can be added to the `docs/` directory as well.

Also, a `README.md` file can also be included in directories within the `include/` and `src/`
directories (and subdirectories). Adding a `README.md` in these directories will add a detailed
description to the directory in the **File List** of Doxygen.

There is a special case as well, when there is a `README.md` in the `docs/` directory, it will be used
as the MAINPAGE for generation of Doxygen documentation. If it doesn't exist and there is a
`README.md` file at the root of the package directory, it will be used as the MAINPAGE. If both files
exist, the `docs/README.md` will be used and the `README.md` will be added as a regular page. Note
that, at the time of writing this, the MAINPAGE can't include an H1 (`#`) or it will cause the tree
view on the docs site to act strange. Generally, I prefer using the `docs/README.md` for the packages
MAINPAGE instead of `README.md` so I can provide specific documentation-related information.

HTML output can be customized by creating some files within the docs directory:

- `stylesheet.css`: A custom stylesheet for the HTML pages generated by Doxygen.
- `header.html`: A custom header for the HTML pages generated by Doxygen.
- `footer.html`: A custom footer for the HTML pages generated by Doxygen.

> **Note:** You can generate the default header and footer that Doxygen uses to customize using the
> `doxygen` command:
>
> ```bash
